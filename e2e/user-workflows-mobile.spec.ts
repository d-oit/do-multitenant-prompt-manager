...
test.describe('Complete Mobile User Workflows', () => {
  ...
  test.describe('Power User Workflows', () => {
    ...
    test('Multi-tenant workflow on mobile', async ({ browser }) => {
      ...
      if (await tenantNav.isVisible()) {
        ...
        const createTenantToggle = page.locator('summary:has-text("Create tenant"), button:has-text("Create")');
  ...
  test.describe('Cross-Feature Integration Workflows', () => {
    ...
    test('Create-to-test prompt workflow', async ({ browser }) => {
      ...
      const createdPrompt = page.locator('.data-table__mobile-card:has-text("Mobile Created Prompt")');
      ...
      if (await form.isVisible()) {
        ...
        if (await input.isVisible()) {
          ...
          if (await submitButton.isVisible()) {
            ...
            const offlineMessage = page.locator(':has-text("offline"), :has-text("connection"), [role="alert"]');
    ...
    test('Large dataset interaction workflow', async ({ browser }) => {
      ...
      await Promise.all(
        Array.from({ length: 50 }, (_, i) =>
          createTestPrompt(testTenant.id, `Performance Test ${i + 1}`, `Large dataset content ${i + 1}`)
        )
      );
    ...
    test('Rapid navigation workflow', async ({ browser }) => {
      ...
      for (let i = 0; i < 3; i++) { // Do multiple rounds
        for (const section of sections) {
          ...
          const heading = page.locator(`h1:has-text("${section}"), h2:has-text("${section}"), .page-title`);
