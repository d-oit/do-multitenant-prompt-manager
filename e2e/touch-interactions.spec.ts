...
test.describe("Touch Interactions and Gestures", () => {
  ...
  test.describe("Bottom Sheet Component", () => {
    ...
    test("Bottom Sheet Opens and Closes", async ({ page }) => {
      ...
      const bottomSheetTrigger = page.locator(
        '[data-testid="bottom-sheet-trigger"], .bottom-sheet-trigger'
      );
    ...
    test("Pull to Refresh Visual Feedback", async ({ page }) => {
      ...
      if (await pullToRefreshContainer.isVisible()) {
        ...
        if (contentBox) {
          ...
          await page.mouse.move(contentBox.x + contentBox.width / 2, contentBox.y + 60, {
            steps: 5
          });
  ...
  test.describe("Command Palette", () => {
    ...
    test("Command Palette Touch Interaction", async ({ page }) => {
      ...
      const commandPaletteTrigger = page.locator(
        '[data-testid="command-palette-trigger"], .command-palette-trigger'
      );
      ...
      if (await commandPaletteTrigger.isVisible()) {
        ...
        if (await results.isVisible()) {
          ...
          if (resultsBox) {
            ...
            await page.mouse.move(
              resultsBox.x + resultsBox.width / 2,
              resultsBox.y + resultsBox.height / 2 - 50,
              { steps: 5 }
            );
            await page.mouse.up();
          }
