...
test.describe("Modern UI Components", () => {
  ...
  test.beforeAll(async () => {
    ...
    testTenant = await createTestTenant(
      `modern-components-test-${uniqueId}`,
      `modern-components-test-${uniqueId}`
    );
    testPrompts = await Promise.all(
      Array.from({ length: 15 }, (_, i) =>
        createTestPrompt(
          testTenant.id,
          `Component Test Prompt ${i + 1}`,
          `Test content for component ${i + 1}`
        )
      )
    );
  });
  ...
  test.describe("Enhanced Data Table", () => {
    ...
    test("Table sorting functionality", async ({ page }) => {
      ...
      if (await sortableHeader.isVisible()) {
        ...
        const sortedIcon = sortableHeader.locator(
          ".data-table__sort-icon--desc, .data-table__sort-icon--asc"
        );
  ...
  test.describe("Theme and Visual Enhancements", () => {
    ...
    test("Consistent spacing and typography", async ({ page }) => {
      ...
      const spacedElements = page.locator(
        ".pm-spacing-1, .pm-spacing-2, .pm-spacing-3, .pm-spacing-4"
      );
