  ...
  test.describe("Keyboard Navigation", () => {
    ...
    test("Skip links work correctly", async ({ page }) => {
      ...
      if (await skipLink.first().isVisible()) {
        ...
        const isInMain =
          (await mainContent.locator(":focus").count()) > 0 ||
  ...
  test.describe("Motion and Animation", () => {
    test("Respects reduced motion preference", async ({ page }) => {
      ...
      const animatedElement = page
        .locator('.mobile-nav__hamburger-button, .animated, [class*="transition"]')
        .first();
    ...
    test("No auto-playing content", async ({ page }) => {
      ...
      const carousels = page.locator(
        '[role="region"][aria-label*="carousel"], [class*="carousel"], [class*="slider"]'
      );

      if ((await carousels.count()) > 0) {
        // If carousels exist, they should have pause controls
        const pauseControls = page.locator(
          'button:has-text("pause"), button:has-text("stop"), button[aria-label*="pause"]'
        );
  ...
  test.describe("Error Handling and Feedback", () => {
      ...
      if ((await forms.count()) > 0) {
        ...
        if (await submitButton.isVisible()) {
          ...
          const errorMessages = page.locator(
            '[role="alert"], .error, [aria-invalid="true"] + *, [class*="error"]'
          );
          ...
          if ((await errorMessages.count()) > 0) {
            ...
            expect(ariaLive === "polite" || ariaLive === "assertive" || role === "alert").toBe(
              true
            );
          }
        }
    ...
    test("Loading states are announced", async ({ page }) => {
      ...
      const loadingIndicators = page.locator(
        '[aria-label*="loading"], [aria-label*="Loading"], .loading, .spinner'
      );
      ...
      if ((await loadingIndicators.count()) > 0) {
        ...
        expect(ariaLabel?.toLowerCase().includes("loading") || ariaLive || role === "status").toBe(
          true
        );
      }
    });
